  ...
  Future<List<EpisodeUnlockModel>> callApi({
    ...
    try {
      ...
      if (response.statusCode == 200) {
        Utils.showLog("Fetch episode unlock API Response => ${response.body}", level: LogLevel.debug);

        final Map<String, dynamic> jsonData = jsonDecode(response.body);

        if (jsonData['status'] == true && jsonData['data'] is List) {
          List<EpisodeUnlockModel> episodeUnlockList =
              (jsonData['data'] as List).map((coinData) => EpisodeUnlockModel.fromJson(coinData)).toList();
        ...
        Utils.showLog("Fetch episode unlock API Status Code Error => ${response.statusCode}", level: LogLevel.error);
      }
    } catch (error) {
      Utils.showLog("Fetch episode unlock API Error => $error", level: LogLevel.error);
    }
    return [];
  }
}
