...
class ReportApi {
  static Future<List<ReportReason>> callApi() async {
    Utils.showLog("Fetch Report Reason API Calling...", level: LogLevel.debug);
    ...
    try {
      ...
      if (response.statusCode == 200) {
        Utils.showLog("Fetch Report Reason API Response => ${response.body}", level: LogLevel.debug);
        ...
        Utils.showLog("Fetch Report Reason API Status Code Error => ${response.statusCode}", level: LogLevel.error);
      }
    } catch (error) {
      Utils.showLog("Fetch Report Reason API Error => $error", level: LogLevel.error);
    }
