...
class Preference {
  ...
  static onSetUserID(String userId) async =>
      await _pref?.write(USER_ID, userId);
  ...
  static onIsAutoCheck(bool isCheck) async =>
      await _pref?.write(IS_CHECK, isCheck);

  static onIsNotification(bool isCheck) async =>
      await _pref?.write(NOTIFICATION, isCheck);

  static onSetDayWiseWatchTime(int day, int watchTime) async =>
      await _pref?.write("$day", watchTime);

  static onSetLastOpenWeekDay(int weekDay) async =>
      await _pref?.write(weekday, weekDay);

  static onSetFcmToken(String fcmToken) async =>
      await _pref?.write(FCM_TOKEN, fcmToken);

  static onSetIdentity(String identity) async =>
      await _pref?.write(IDENTITY, identity);

  static onSetLastWeekWatchTime(int watchTime) async =>
      await _pref?.write(lastWeekWatchTime, watchTime);
  ...
  static onSetLanguage(int language) async =>
      await _pref?.write(LANGUAGE, language);

  // *** Watch Ad History Database ***

  static int? get nextAdTaskTimes => _pref?.read(nextAdTaskTime);

  static onSetNextAdTaskTime(int nextAdTaskTimes) async =>
      await _pref?.write(nextAdTaskTime, nextAdTaskTimes);

  static String? get lastRewardDates => _pref?.read(lastRewardDate);

  static onSetLastRewardDate(String lastRewardDates) async =>
      await _pref?.write(lastRewardDates, lastRewardDates);
